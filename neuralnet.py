...
def plot_wine():
    ...
    train_sizes, train_scores, valid_scores, y_test, y_pred = run_neural_net(wine_X, wine_y,finalWineX, finalWineY)

    plt.figure()
    plt.title("Multi-Layer Perceptron Learning Curves: Wine Data")
...
def plot_car():
    ...
    car_X, finalCarX = scaler.transform(car_X), scaler.transform(finalCarX)

    train_sizes, train_scores, valid_scores, y_test, y_pred = run_neural_net(car_X, car_y, finalCarX, finalCarY)

    plt.figure()
    plt.title("Multilayer Perceptron Learning Curves: Car Data")
    plt.xlabel("Training examples")
    plt.ylabel("Score")
    train_scores_mean = np.mean(train_scores, axis=1)
    train_scores_std = np.std(train_scores, axis=1)
